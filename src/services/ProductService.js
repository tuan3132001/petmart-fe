...
export const getAllProduct = async (search, limit) => {
    let res = {}
    if (search?.length > 0) {
        res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-product?filter=name&filter=${search}&limit=${limit}`)
    } else {
...
export const getAllProductSelled = async () => {
     ...
     const res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-product`)
